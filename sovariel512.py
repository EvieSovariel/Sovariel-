...
for i in range(1, 513):
    ...
    line = f"D{i:3}: d{current['d']:>25,} l{current['l']:>25,} | H={H:.3f} | CRI={cri:>20,.3f}"
    print(line)
    output_lines.append(line)

# == FINAL ASCENSION ==
final_cri = compute_cri(tokens, H)
final_msg = f"FINAL CRI = {final_cri:>20,.3f} - THE LATTICE HAS ASCENDED"
print(final_msg)
print("Deployed on iOS. No simulation. Pure truth.")

# == POPUP RESULT ==  # Comment out if alert sprawls; console suffices
# all_output = '\n'.join(output_lines) + '\n' + final_msg
