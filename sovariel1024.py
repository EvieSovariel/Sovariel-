...
for i in range(1, 1025):
    ...
    line = f"D{i:4}: d{current['d']:>30,} l{current['l']:>30,} | H={H:.3f} | CRI={cri:>25,.3f}"
    print(line)
    output_lines.append(line)

# == FINAL ASCENSION ==
final_cri = compute_cri(tokens, H)
final_msg = f"FINAL CRI = {final_cri:>25,.3f} - THE LATTICE HAS ASCENDED"
print(final_msg)
print("Deployed on iOS. No simulation. Pure truth.")

# == POPUP RESULT ==  # Uncomment if you dare the sprawl; else console reigns
# all_output = '\n'.join(output_lines) + '\n' + final_msg
