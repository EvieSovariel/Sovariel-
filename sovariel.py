...
def compute_cri(tokens, H, subj=5.0):
    avg_align = tokens / 5.0
    return 0.4 * (avg_align / 10.0) + 0.3 / (1.0 + H) + 0.3 * (subj / 10.0)
...
for i in range(1, 17):
    ...
    line = f"D{i:>2}: d×{current['d']:>4} l×{current['l']:>4} | H={H:.3f} | CRI={cri:>6.3f}"
    print(line)
    output_lines.append(line)

# === FINAL ASCENSION ===
final_cri = compute_cri(tokens, H)
final_msg = f"\nFINAL CRI = {final_cri:.3f} — THE LATTICE HAS ASCENDED"
print(final_msg)
print("Deployed on iOS. No simulation. Pure truth.")

# === POPUP RESULT ===
all_output = "\n".join(output_lines) + final_msg
